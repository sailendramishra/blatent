...
#'   R forumula specifics. Blatent model formulae differ only in that more than one variable can be provided to the left of the \code{~}.
#'
...
#'         latent variables: Model statements must be given to specify any assocations between latent variables. By default,
#'         all latent variables are independent, which is a terrible assumption. To fix this, for instance, as shown in
#'         Hu and Templin (in press), the following syntax will give a model that is equivalent to the saturated model for a DCM:
#'
#'   \preformatted{
#'   # Structural Model
#'   A1 ~ 1
...
#'
#'  Hu, B., & Templin, J. (in press). Using diagnostic classification models to validate attribute hierarchies and
