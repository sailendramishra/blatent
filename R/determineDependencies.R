determineDependencies = function(modelVector){
  ...
  colnames(predictorMatrix) = unlist(lapply(X = modelVector, FUN = function(x) return(all.vars(terms(x)[[2]]))))
  rownames(predictorMatrix) = colnames(predictorMatrix)

  model = 1
  for (model in 1:length(modelVector)){
    lhs = all.vars(terms(modelVector[[model]])[[2]])
    rhs = all.vars(terms(modelVector[[model]])[[3]])
    predictorMatrix[rhs, lhs] = 1
  }

  dependList = unlist(lapply(X = modelVector, FUN = function(x) return(all.vars(terms(x)[[2]]))))
  predOrder = NULL
  ...
  while (length(dependList) > 0){
    ...
    if (is.null(predOrder)){
      ...
      clearList = unlist(lapply(X = modelVector, FUN = function(x){
        if (length(which(all.vars(terms(x)[[3]]) %in% predOrder)) == length(all.vars(terms(x)[[3]]))){
          return(TRUE)
        } else {
          return(FALSE)
        }
      ...
      addList = unlist(lapply(X = modelVector, FUN = function(x) return(all.vars(terms(x)[[2]]))))[clearList]
